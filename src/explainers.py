...
class GraphSVX():
    ...
    def explain(self,
        ...
        for node_index in node_indexes:
            ...
            genes, contribution = self.rank_top_gene(phi, feat_idx, true_pred, multiclass, self.F)
                
            gene_list.extend(genes)
            contribution_list.extend(contribution)
        ...
        return phi_list,gene_list,contribution_list

    def explain_simulated(self,
                node_indexes=[0],
                hops=2,
                num_samples=10,
                info=True,
        ...
        for node_index in node_indexes:
            ...
            # Compute true prediction for original instance via explained GNN model
            with torch.no_grad():
                true_pred = self.model(
                    self.data.x, 
                    self.data.edge_index).exp()[node_index]
            ...
            # Print information
            if info:
                self.print_info_simulated(D, node_index, phi, feat_idx,
                                true_pred, base_value, multiclass)
    ...
    def rank_top_gene(self, phi, feat_idx, true_pred, multiclass, n):
        """
        Displays and saves marker gene. 
        """
        gene=[]
        contribution=[]
        ...
        _, idxs = torch.topk(torch.from_numpy(
            np.abs(pred_explanation[:self.F])), n)
        vals = [pred_explanation[idx] for idx in idxs]
        influential_feat = {}
        for idx, val in zip(idxs, vals):
            influential_feat[feat_idx[idx]] = val
        # print('Most rank top {} genes: '.format(n), [
        #         (item[0].item(), item[1].item()) for item in list(influential_feat.items())])
        for item in list(influential_feat.items()):
            gene.append(item[0].item())
            contribution.append(item[1].item())
        return gene,contribution
