...
##################################################################
# Node Classification on proportion datasets
##################################################################

def train_simulated(data, model, args):
    """ Train model on synthetic datasets, for node classification
    """
    # Optimizer 
    opt = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.num_epochs):
        ...
        # Evaluate on validation set
        if epoch % 10 == 0:
            train_MSE = MSE(data, model, data.train_mask)
            val_MSE = MSE(data, model, data.val_mask)
            print("Epoch {}. Loss: {:.4f}. Train MSE: {:.4f}. Val MSE: {:.4f}.".format(
                epoch, total_loss, train_MSE, val_MSE))
    total_loss = total_loss / data.x.shape[0]

def MSE(data, model, mask):
    model.eval()

    with torch.no_grad():
        pred = model(data.x, data.edge_index).exp()
        pred = pred[mask]
        label = data.y[mask]
